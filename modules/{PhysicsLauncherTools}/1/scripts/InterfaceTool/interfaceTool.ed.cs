...
{
    %levelImage = %this.levelTabSelectedWorld.LevelImageList[0];
    %levelLockedImage = %this.levelTabSelectedWorld.LevelLockedImage[0];
    if (%levelImage $= "")
        return;
...
{
    %count = %this.worldData.getCount();
    for (%i = 0; %i < %count; %i++)
    {
        %world = %this.worldData.getObject(%i);
        for (%j = 0; %j < %world.WorldLevelCount; %j++)
        {
            %world.LevelImageList[%j] = %icon;
            %world.LevelLockedImage[%j] = %lockedIcon;
        }
    }
...
function InterfaceTool::populateLevelPane(%this, %index)
{
    ...
    {
        %pageName = "Page " @ %i + 1;
        LevelSelectBackground.Image = %world.WorldBackground[%i];
        for(%j = 1; %j < 16; %j++)
        {
            %levelButton = "LevelSelectButton" @ %j;
            if (%j < 3)
                %levelButton.NormalImage = %world.LevelImageList[0];
            else
                %levelButton.NormalImage = %world.LevelLockedImage[0];
        }
...
function It_AssetSelectButton::setSelectedAsset(%this, %asset)
{
    %nameTag = strreplace(%this.getName(), "SelectBtn", "");
    %dropdown = %nameTag @ "StateDropdown";
        ...
        if (%edit.preview !$= "")
        {
            ...
            switch$(%previewType)
            {
                case "GuiImageButtonCtrl":
                    ...
                    %edit.preview.NormalImage = %asset;
                    ...
                    switch (%index)
                    {
                        case 0:
                            %edit.preview.NormalImage = %asset;
                        case 1:
                            %edit.preview.HoverImage = %asset;
                        case 2:
                            %edit.preview.DepressedImage = %asset;
                        case 3:
                            %edit.preview.InactiveImage = %asset;
                    }

                case "GuiSpriteCtrl":
                    %edit.preview.setImage(%asset);
            }
        }
    }
    eval(%edit.stateControl  @ "." @ %edit.stateField[%index] @ " = \"" @ %asset @ "\";");
    InterfaceTool.saveAllGuiObjects();
}
...
function It_LargeRewardSelectButton::setSelectedAsset(%this, %asset)
{
    %nameTag = strreplace(%this.getName(), "SelectBtn", "");
    %dropdown = %nameTag @ "StateDropdown";
...
function setStateAsset(%dropdown, %asset)
{
