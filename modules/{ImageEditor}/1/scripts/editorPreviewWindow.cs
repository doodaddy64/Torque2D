...
function createGridOverlay(%guiControl, %position, %extent, %rows, %columns, %cellSize, %shadeNoneCells)
{
    ...
    if (%shadeNoneCells == true)
    {
        ...
        %rowGreyBox = new GuiControl()
        {
            ...
            HorizSizing="left";
            VertSizing="top";
            Position=mRound(%posX) SPC mRound(%posY);
            Extent=mRound(%width) SPC mRound(%height);
            Visible="1";
        };
    ...
    for (%i = 0; %i < %rows; %i++)
    {
        %rowBox = new GuiControl()
        {
            ...
            VertSizing="top";
            Position=mRound(%posX) SPC mRound(%posY);
            Extent=mRound(%rowWidth) SPC mRound(%rowHeight) + 1;
            Visible="1";
        };
    ...
    for (%i = 0; %i < %columns; %i++)
    {
        %columnBox = new GuiControl()
        {
            ...
            VertSizing="top";
            Position=mRound(%posX) SPC mRound(%posY);
            Extent=mRound(%columnWidth) + 1 SPC mRound(%columnHeight);
            Visible="1";
        };
