...
class ASResource
{
	...
	public:
		...
		static const string AS_SWITCH, AS_SWITCH_S, AS_CASE, AS_DEFAULT;
		...
		static const string AS_S_EQUAL, AS_NOT_S_EQUAL, AS_EQUAL, AS_PLUS_PLUS, AS_MINUS_MINUS, AS_NOT_EQUAL, AS_GR_EQUAL, AS_GR_GR_GR, AS_GR_GR;
...
class ASBase
{
	...
	protected:
		...
		//??????			|| (isSharpStyle() && ch == '$') // DCP: may be used in torque script
					|| (isSharpStyle() && ch == '%') // DCP: may be used in torque script
					);
		}
		...
		bool isCharPotentialHeader(const string& line, size_t i) const {
			...
			if ((unsigned)line[i] > 127) return false;

			// handle 'switch$'
			if (isSharpStyle() && (prevCh == 'h') && (line[i] == '$'))
				return true;
