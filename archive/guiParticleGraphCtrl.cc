...
void GuiParticleGraphCtrl::onRender(Point2I offset, const RectI &updateRect)
{
	...
	for (int k = 0; k < MaxPlots; k++)
	{
		...
		F32 ScaleX = (F32(getExtent().x) / (graphExtent.x*(1.0f + (mPlots[k].mGraphScale))));
		F32 ScaleY = (F32(getExtent().y) / (graphExtent.y*(1.0f + (mPlots[k].mGraphScale))));
		...
		if((mPlots[k].mGraphType == Point) || (mPlots[k].mGraphType == Polyline))
		{
         S32 posX;
            ...
            posX += (S32)(getExtent().x * (mPlots[k].mGraphScale));
				posY /= (S32)(1.0f + (mPlots[k].mGraphScale));
            ...
            mPlots[k].mNutList.push_back( Point2F((F32)posX, (F32)posY) );
            ...
            // draw the seleciton nut
               ...
               if((k == mSelectedPlot && sample == mLastSelectedPoint) || (mRenderAllPoints == true))
                  ...
                  S32 textWidth = (S32)font->getStrWidth((const UTF8*)number);;
				      ...
				      {
                      posX += (S32)(textWidth/1.5f);
				      } else if((posX + (textWidth * 1.8f)) > (comparePos.x + getExtent().x) || ((posX + textWidth) >= (S32)res.w))
				      {
                      posX -= (S32)(textWidth * 1.5f);
				      }
...
Point2I GuiParticleGraphCtrl::findHitNut( Point2I hitPoint )
{
   for(S32 i = 0; i < MaxPlots; i++)
   {
      ...
      {
	     if( inNut (Point2I( (S32)mPlots[i].mNutList[j].x, (S32)mPlots[i].mNutList[j].y), hitPoint.x, hitPoint.y) )
		 {
...
Point2F GuiParticleGraphCtrl::convertToGraphCoord(S32 plotID, Point2I v)
{
   ...
   v.y = getExtent().y - v.y;
   resultV = Point2F((F32)v.x, (F32)v.y);
   resultV.x /= getExtent().x; 
   resultV.y /= getExtent().y;
   resultV.x *= getGraphExtent(plotID).x;
   resultV.y *= getGraphExtent(plotID).y;
   resultV.x *= 1.0f + (mPlots[plotID].mGraphScale*2.0f);
   resultV.y *= 1.0f + (mPlots[plotID].mGraphScale*2.0f);
