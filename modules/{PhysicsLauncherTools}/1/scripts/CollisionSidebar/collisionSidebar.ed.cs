    ...
    switch$ (%type)
    {
        case "All":
            for (%i = 0; %i < %this.projectileSet.getCount(); %i++)
            {
                ...
                %buttonData = %i SPC %object SPC %type SPC %objCommand;
...
function CollisionSidebar::selectObject(%this, %data)
{
    ...
    if (%type $= "World")
    {
        %temp = %type @ getWord(%data, 3);
        %type = %temp;
        %command = getWord(%data, 4);
    }
    else
        %command = getWord(%data, 3);
...
{
    PhysicsLauncherTools::copyCollisionShapes(%object, %this.lastObject);
