...
{    
    ...
    if (%scaleFromCenter)
    {
        %anchorPoint = %shape.position;
    }
    else
    {
        %anchorPointX = %this.dragStartShapePosition.x + (%this.dragStartShapeSize.x * -1 * %handlePositionX);
        %anchorPointY = %this.dragStartShapePosition.y + (%this.dragStartShapeSize.y * -1 * %handlePositionY);
        %anchorPoint = %anchorPointX SPC %anchorPointY;
    }
    ...
    // Transform the shape position
    %startPosition = %this.dragStartShapePosition;
    %positionTransformVector = Vector2Sub(%startPosition, %anchorPoint);
    %positionTransformVector.x *= (%scaleX - 1);
    %positionTransformVector.y *= (%scaleY - 1);

    %shape.setPosition(%startPosition.x + %positionTransformVector.x/2, %startPosition.y + %positionTransformVector.y/2); 
}
...
{
    // Ignore event if we are already dragging another control
    if (CEPreviewWindow.lockedControl != %this && isObject(CEPreviewWindow.lockedControl))
    {
        CEPreviewWindow.lockedControl.onMouseDragged(%modifier, %mousePoint, %mouseClickCount);
        return;    
    } 
