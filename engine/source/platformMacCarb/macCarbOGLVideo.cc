...
//      attr[i++] = AGL_FULLSCREEN;
   ...
   // DWB: don't use GDHandles anymore (to support OSX 10.7), pass NULL
   fmt = aglChoosePixelFormat(NULL, 0, attr);
   dumpAGLDebugStr();
...
void OpenGLDevice::initDevice()
{
   ...
   platState.cgDisplay = chooseMonitor();
...
bool OpenGLDevice::enumMonitors()
{
   ...
   CGError _err = CGGetActiveDisplayList (32, _displayIDs, &_displayCount);
   ...
   // DWB 8-1-11: above, rewrote the following code to support OSX 10.7
   /*   
...
//------------------------------------------------------------------------------
CGDirectDisplayID OpenGLDevice::chooseMonitor()
{
...
bool OpenGLDevice::setScreenMode( U32 width, U32 height, U32 bpp, bool fullScreen, bool forceIt, bool repaint )
{
   ...
   CGDirectDisplayID prevDisplay = platState.cgDisplay;
   platState.cgDisplay = chooseMonitor();
   AssertFatal(platState.cgDisplay,"We chose a null monitor? Panic!");
   
   // if we're changing screens, we must know what the new one is capable of
   if(prevDisplay != platState.cgDisplay)
   {
      enumDisplayModes(platState.cgDisplay);
      getGLCapabilities();
   }
   ...
   else
   {
      ...
      platState.appWindow = MacCarbCreateOpenGLWindow( platState.cgDisplay, newRes.w, newRes.h, isFullscreenWindow );
      if(!platState.appWindow)
      {
...
Resolution Video::getDesktopResolution()
{
   ...
   CGRect r = CGDisplayBounds(platState.cgDisplay);
   
   res.w = r.size.width;
   res.h = r.size.height;
   // DWB 8-2-11: DM.. fcns deprecated and removed, Quartz doesn't give true alternative for getting bpp without parsing mode strings
   //             Due to our minimum HW and OS req's, it is ok to assume 32-bit display device and circumvent the whole bpp issue
   res.bpp = 32;//(**(**platState.hDisplay).gdPMap).pixelSize;
...
{
   /* DWB 8-1-11 : temp disable -- needs porting to quartz
