...
function ScheduleManager::rescheduleEvent(%this, %eventObject)
{
   ...
   %eventID = schedule(%time, %object, %method, %targetID, %param1, %param2, %param3, %param4);
   //echo(" $$ ScheduleManager::scheduleEvent("@%time@", "@%object@", "@%method@", "@%targetID@", "@%param1@", "@%param2@", "@%param3@", "@%param4@", "@%eventID@", "@%handle@")");
   
   %eventObject.eventID = %eventID;
   
   // Add our new schedule data to our SimSet
   %this.ScheduleSet.add(%eventObject);

   // Return the event handle
   return %eventObject.handle;
}
