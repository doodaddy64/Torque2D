...
function WorldTool::refreshWorldList(%this)
{
    ...
    {
        %worldName = %this.currentWorlds[%i].WorldName;
...
function WorldTool::createUnassignedWorldButton(%this)
{
    ...
    %worldSelectBtnPreview = new GuiSpriteCtrl()
    {
        ...
        Active="0";
        hovertime="1000";
        Image="{PhysicsLauncherTools}:unusedLevelsImage";
    };
...
function WorldTool::finalizeWorldAdd(%this, %data)
{
    ...
    %newWorld.WorldName = getWord(%nameData, 1) SPC getWord(%nameData, 2);
...
function WorldTool::checkValidLevelFileName(%this, %fileName)
{
    ...
    {
        // Show message dialog
        Plt_ConfirmDeleteGui.display("Level name cannot be empty!", 
    ...
    {
        // Show message dialog
        Plt_ConfirmDeleteGui.display("Level name cannot contain spaces!", 
        "WorldTool", "resetLevelName", %this.selectedLevelIndex);
    ...
    {
        // Show message dialog
        Plt_ConfirmDeleteGui.display("Level name contains invalid symbols!", 
        "WorldTool", "resetLevelName", %this.selectedLevelIndex);
...
function WorldTool::SetSelectedWorldButton(%this, %position)
{
    ...
    %worldName = %this.currentWorlds[%this.worldIndex].WorldName;
...
function Wt_LevelNameEdit::onValidate(%this)
{
    ...
    if (%validName && !%duplicate)
    {
        ...
        if (PhysicsLauncherTools::renameCurrentLevelFile(%oldName, %newLevelName, %oldLevel))
        {
            ...
            WorldTool.lastLevelName = %newLevelName;
            WorldTool.saveData();
            WorldTool.getWorldLevels();
            WorldTool.refreshWorldList();
            WorldTool.refreshLevelList();
            WorldTool.SetSelectedWorldButton(WorldTool.worldIndex);
            WorldTool.SetSelectedLevelButton(WorldTool.selectedLevelIndex);
        }
    }
    else if (%duplicate)
    {
...
{
    %worldName = WorldTool.CurrentWorlds[WorldTool.selectedWorld].WorldName;
...
function Wt_WorldMoveDownBtn::onClick(%this)
{
    ...
    WorldTool.refresh();
    WorldTool.WorldListContainer.scrollToButton(%this.index);
    PhysicsLauncherToolsEventManager.postEvent("_WorldButtonUpdateComplete", %this.index);
}
...
function Wt_LevelEditToolBtn::onClick(%this)
{
    ...
    LevelBuilderToolPresenter::loadLevel(WorldTool.worldIndex SPC WorldTool.lastLevelName);
...
{
    alxStopAll();
    WorldTool.playingMusic = alxPlay(WorldTool.loadedLevel.music);
}
...
{
    alxStop(WorldTool.playingMusic);
}
