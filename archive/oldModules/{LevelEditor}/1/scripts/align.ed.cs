...
function AlignTools::alignToCamera( %objects, %scene, %side, %dir )
{
   %camPos = getWord( %scene.cameraPosition, %dir );
   %camHalfSize = getWord( %scene.cameraSize, %dir ) * 0.5;
...
function AlignTools::distributeToCamera( %objects, %scene, %side, %dir )
{
   %count = %objects.getCount();
   if( %count < 2 )
      return;
...
function AlignTools::matchSizeToCamera( %objects, %scene, %dir )
{
   if( %dir == 2 )
   {
      AlignTools::matchSizeToCamera( %objects, %scene, 0 );
      AlignTools::matchSizeToCamera( %objects, %scene, 1 );
      return;
   }
   
   %size = getWord( %scene.cameraSize, %dir );
...
function AlignTools::spaceToCamera( %objects, %scene, %dir )
{
   %count = %objects.getCount();
   if( %count < 2 )
      return;
