...
function CollisionShapePreview::addPolygonCollisionShape(%this, %points, %boundingBox)
{
    ...
    %sizeX = (getWord(%boundingBox, 2) - getWord(%boundingBox, 0))/2;
    %sizeY = (getWord(%boundingBox, 3) - getWord(%boundingBox, 1))/2;
    
    // Transform the points to the local space of the new shape
    %positionTransform = %centerX SPC %centerY;
    %scaleTransform = "1 1";
    //%scaleTransform = %sizeX/%this.previewObject.size.x SPC %sizeY/%this.previewObject.size.y;
