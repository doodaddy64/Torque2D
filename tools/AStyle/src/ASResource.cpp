...
const string ASResource::AS_SWITCH_S = string("switch$");
...
const string ASResource::AS_S_EQUAL = string("$=");			// torque script.
const string ASResource::AS_NOT_S_EQUAL = string("!$=");	// torque script.
...
void ASResource::buildHeaders(vector<const string*>* headers, int fileType, bool beautifier)
{
	...
	if (fileType == SHARP_TYPE)
	{
		// DCP: hack to solve bad sorting issue later on...
		headers->insert(headers->begin(), &AS_SWITCH_S);	
	}
}
...
{
	nonAssignmentOperators->push_back(&AS_S_EQUAL);
	nonAssignmentOperators->push_back(&AS_NOT_S_EQUAL);
...
void ASResource::buildOperators(vector<const string*>* operators)
{
	...
	operators->push_back(&AS_S_EQUAL);
	operators->push_back(&AS_NOT_S_EQUAL);
