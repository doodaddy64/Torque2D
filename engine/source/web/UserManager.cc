...
void UserManager::Update()
{
    switch(m_UserState)
    {
        ...
        case UserState_UserOfflineMode:
        case UserState_GettingUserInfo:
        case UserState_GetUserCredentials:
        case UserState_UserLoggedIn:
        case UserState_RequestOfflineMode:
        {
            Con::errorf("Unhandled case in UserManager::Update. Bad state has been reached");
            break;
        }
    }
}
...
void UserManager::onRequestFinishedCallback(S32 id, S32 responseCode, const char* response)
{
    ...
    if(m_UserState == UserState_CheckUserLoggedIn)
    {
        case UserState_CheckUserLoggedIn:
            {
                ...
                else
                {
                    ...
                    FileStream fstream;
                    fstream.open(buffer, FileStream::Write);
        ...
        case UserState_UserOfflineMode:
        {
            Con::errorf("Unhandled case in UserManager::onRequestFinishedCallback. Bad state has been reached");
            break;
        }
